...
void _cpy_dec(s21_decimal d, s21_decimal *r) {
  if (r) {
    for (int i = 0; i < 4; i++) r->bits[i] = d.bits[i];
  }
}
    ...
    if (src != 0) {
      ...
      if (rf >= 1E-28 && rf < 79228162514264337593543950335.0f) {
        ...
        if (binexp <= 95) {
          ...
          setScale(dst, scale);
          if (shift <= 0) _double_to_dec(rf, dst);
          if (neg) setBit(dst, 127);
        } else {
          rv = 1;
        }
      } else {
