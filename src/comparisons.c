...
int s21_is_equal(s21_decimal first, s21_decimal second) {
  ...
  for (int i = 0; i < 3 && allZero == 1; ++i)
    if (first.bits[i] != 0 || second.bits[i] != 0) allZero = 0;

  if (!allZero) {
    for (int i = 0; i < 4 && equal == 1; ++i)
      if (first.bits[i] != second.bits[i]) equal = 0;
  }
...
unsigned int binaryToInt(int* binary, int shift) {
  ...
  if (binary != NULL) {
    for (int i = 0; i < 32; ++i)
      if (binary[32 * shift + i] == 1) number += pow(2, i);
  }
  return number;
}
  ...
  if (numberCopy != NULL) {
    for (int i = 0; i < 7; ++i) {
      ...
      if (binary != NULL) {
        ...
        for (int j = 0; j < 32; ++j) numberCopy[i * 32 + j] = binary[j];
        free(binary);
      }
    }

    int zeroCounts = 0;
    while (numberCopy[zeroCounts] == 0) ++zeroCounts;
    ...
    if (numberCopyMultipliedBy2 != NULL) {
      ...
      if (result != NULL) {
        ...
        for (int i = 0; i < 7 * 32 - zeroCounts - 1; ++i) {
          int localResult =
              numberCopy[zeroCounts + i] + numberCopyMultipliedBy2[i] + memory;
...
void normilize(
    s21_decimal numberWithMaxScale, s21_decimal otherNumber,
    unsigned int** newNumber)  // newNumber = otherNumber * 10^maxScale
{
  int scale = getScale(numberWithMaxScale) - getScale(otherNumber);

  for (int i = 0; i < 3; ++i) (*newNumber)[i] = otherNumber.bits[i];

  for (int i = 0; i < scale; ++i) multiplyBy10(newNumber);
}

int isZero(s21_decimal number) {
  int result = 1;
  for (int i = 0; i < 3 && result == 1; ++i)
    if (number.bits[i] != 0) result = 0;
...
int s21_is_less(s21_decimal first, s21_decimal second) {
  ...
  if (firstSign != secondSign) {
    ...
    if (isZero(first) || isZero(second)) {
      if (isZero(first) && !isZero(second))
        resultByModule = 1;
      else if (!isZero(first) && isZero(second))
        resultByModule = 0;
      ...
      if (newNumber != NULL) {
        for (int i = 0; i < 7; ++i) newNumber[i] = 0;
        s21_decimal* numberWithMaxScale =
            getScale(first) >= getScale(second) ? &first : &second;
        ...
        if (newNumber[3] != 0 || newNumber[4] != 0 || newNumber[5] != 0 ||
            newNumber[6] != 0) {
          resultByModule = numberWithMaxScale == &first ? 1 : 0;
        } else {
          for (int i = 2; i >= 0 && resultByModule == -1; --i) {
            if (numberWithMaxScale->bits[i] < newNumber[i])
              resultByModule = numberWithMaxScale == &first ? 1 : 0;
            else if (numberWithMaxScale->bits[i] > newNumber[i])
              resultByModule = numberWithMaxScale == &first ? 0 : 1;
          }
        }
        free(newNumber);
      }
    }
    result = resultByModule == -1 ? 0
             : firstSign == 0     ? resultByModule
                                  : 1 - resultByModule;
  }
  return result;
}
